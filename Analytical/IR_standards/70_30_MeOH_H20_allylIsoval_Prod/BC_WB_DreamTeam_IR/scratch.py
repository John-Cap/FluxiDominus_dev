...
class IrStandard:
    def __init__(self, mainDir, smallValueThreshold=0.05, trimLeft=0, trimRight=0, gradientPercentile=90, thresholdDistance=100, minRangeWidth=50):
        """
    ...
    def analyzeNewScan(self, rawWavenumbers, rawIntensities, referenceZ=None):
        ...
        if processedScan[1].shape != self.Z.shape[1]:
...
if __name__ == "__main__":
    ...
    for i, testSpectrum in enumerate(test_cases):
        ...
        trueYield = bestMatchIndex/((len(irAnalysis.Z)) - 1)  # Normalize to range 0-1
